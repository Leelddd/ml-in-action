...
def load():
    data = []
    with open('data/retail.csv') as f:
        lines = f.readlines()
        for line in lines:
            items = line.split(',')
...
if __name__ == '__main__':
    ...
    # tree.disp()
    print(type(findPrefixPath(1,item2cnt[1][1])))
    for i in range(50):
        for k, v in findPrefixPath(i, item2cnt[i][1]).items():
            if (v > 1):
                print(k, v)

    myFreqList = []
    # print(mineTree(tree, item2cnt, 5, set([]), myFreqList))
